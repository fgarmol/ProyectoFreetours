...
/**
 * Devuelve la lista de guías para el select.
 * Si la ruta ya tiene un guía asignado y este no figura en la lista filtrada,
 * se agrega un objeto con el id y el nombre real (usando ruta.guia_nombre, si existe).
 */
function obtenerGuias(ruta) {
  ...
  if (ruta.guia_id) {
    ...
    if (!existe) {
      // Se usa el nombre real asignado, asumiendo que está en la propiedad `guia_nombre`
        ...
        <tr v-for="ruta in rutasPaginadas" :key="ruta.id">
          ...
          <td>
            <select v-model="ruta.guia_id" @change="asignarGuia(ruta)">
              ...
              <option 
                v-for="guia in obtenerGuias(ruta)" 
                :key="guia.id" 
                :value="guia.id">
                {{ guia.nombre }}
              </option>
